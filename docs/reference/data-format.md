---
title: Data Format
description: "Learn how to customize your chart data format, from text format to encoding."
sidebar_position: 12
---
...
- [Awesome data format](#awesome-text-format) (recommended)  is the easiest format that we created for you at Image-Charts. It has floating point numbers, **no lower-upper limitation** and **automatic scaling**. If you never used Image-Charts before, **start with this one**.
...
**`<data>`** Each series is one or more comma-separated values. Separate multiple series using a pipe character (`|`). Values are floating point numbers from -infinity to +infinity. Empty value and the underscore character ( `_` ) is considered a `null` value.
